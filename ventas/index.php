...
<html lang="en">
  ...
  <body>
    ...
    <div class="container" style="background:#FCF7F6; ">
			...
			<form method="post" target="_blank" action="guardar.php">
				<div class="row">
				  	<div class="col-md-4">
					...
					<div class="pull-right">
						<div class="col-md-4">
							<div class="input-group">
								...
								<input type="text" class="form-control" name="efectivo" id="efectivo" placeholder="Efectivo" autocomplete="off" onkeyup="vuelto()">		
								
							</div>
							<!--<input type="button" class="btn btn-warnig" onclick="vuelto()" id="boton1" value="Calcular"> -->	
						</div>
						<div class="col-md-4">
							...
							<div class="pull-right">
							    <div class="input-group">
							    	<div class="input-group-addon">
							    	    <input type="button" onclik="Datos()" id="calcular" value="ver">
							    	</div>
							        <!--<input type="text" class="form-control" name="vuelto" id="vuelto" placeholder="Vuelto" autocomplete="off"  >	-->
								<div id="resultado" class="btn btn-warning"></div><!-- Carga los datos ajax vuelto-->
						...
						<div class="col-md-4">
							...
							<button  type="submit" name="guardar" id="guardar" class="btn btn-default">
						...
						<div class="col-sm-12">
							<div class="form-group">
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="radio" name="radio" id="radiominorista" value="MINORISTA" checked>
									<label class="form-check-label" for="radiominorista"> Minorista</label>

									<input class="form-check-input" type="radio" name="radio" id="radiomayorista" value="MAYORISTA">
									<label class="form-check-label" for="radiomayorista"> Mayorista</label>

									<input class="form-check-input" type="radio" name="radio" id="radiocantina" value="CANTINA">
									<label class="form-check-label" for="radiocantina"> Cantina</label>
								</div>
							</div>
					  	</div>
					  	<div class="form-group">
   ...
   <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	...
	<!-- script para abrir el modal al cargar la pagina-->
	<script type="text/javascript" src="js/modal_venta.js"></script>
	<!-- script para abrir el JS para el cacular el vuelto
	<script type="text/javascript" src="vuelto/funciones.js"></script>-->
	...
	<script>
	    //funcion para agragar el producto selecionado a la grilla de ventas---------------------------------------
		function agregar (id)
		{
		    //pasa el precio minorista-----------------------------------------------------
			if(document.getElementById('radiominorista').checked){
			...
			//pasa el precio mayorista------------------------------------------------------------------
			if(document.getElementById('radiomayorista').checked){
			    var precio_venta=$('#precio_mayori_'+id).val();
			    var cantidad=$('#cantidad_'+id).val();
			    //Inicia validacion
			    if (isNaN(cantidad))
			    {
			...
			//pasa el precio cantina----------------------------------------------------------------------------
			if(document.getElementById('radiocantina').checked){
			    var precio_venta=$('#precio_cantina_'+id).val();
			    var cantidad=$('#cantidad_'+id).val();
			    //Inicia validacion
			    if (isNaN(cantidad))
			    {
		...
		//funcion para eliminar productos ya añadidos a la grilla de ventas----------------------------------
		...
		//funcion para enviar los datos al ajax para que guarde las ventas y genere el pdf----------------------
